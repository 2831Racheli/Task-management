...
// הקפידי שהכתובת כאן תהיה הכתובת של השרת ב-Render!
axios.defaults.baseURL = "https://task-management-zx5e.onrender.com";
...
axios.interceptors.response.use(
    response => response,
    error => {
        // אם זו שגיאת 401 והיא לא הגיעה מניסיון התחברות
...
export default {
  ...
  addTask: async (name, categoryId) => {
    const result = await axios.post('/items', { name, isComplete: false, categoryId });
    return result.data;
  },
