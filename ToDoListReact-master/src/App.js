...
function App() {
  ...
  async function loadData() {
    if (!user) return;
    try {
        const [tasksList, categoriesList] = await Promise.all([service.getTasks(), service.getCategories()]);
        setTodos(tasksList);
  ...
  const handleAuth = async (e) => {
    ...
    try {
        if (authData.isLogin) {
            ...
            Swal.fire({ icon: 'success', title: 'נרשמת!', text: 'עכשיו התחבר' });
            setAuthData({ ...authData, isLogin: true });
        }
    } catch (err) { Swal.fire({ icon: 'error', title: 'שגיאה בפרטים' }); }
  };
  ...
  const handleDeleteTask = async (id) => {
    ...
    try { await service.deleteTask(id); } catch (e) { setTodos(original); }
  };
    ...
    return (
      <div className="authContainer">
        <div className="authCard">
          <h2>{authData.isLogin ? "כניסה" : "הרשמה"}</h2>
          ...
          <p className="authToggle" onClick={() => setAuthData({...authData, isLogin: !authData.isLogin})}>
            {authData.isLogin ? "צור חשבון" : "התחבר כאן"}
          </p>
        </div>
      </div>
    );
      ...
      <aside className="sidebar">
        <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            <ul className="categoryList">
                <li className="categoryItem" style={{ backgroundColor: selectedCategory === null ? '#D6CCC2' : 'transparent' }} onClick={() => setSelectedCategory(null)}>📂 כל המשימות</li>
                {categories.map(cat => (
                    <li key={cat.id} className="categoryItem" style={{ backgroundColor: selectedCategory === cat.id ? '#D6CCC2' : 'transparent' }} onClick={() => setSelectedCategory(cat.id)}>🏷️ {cat.name}</li>
                ))}
            </ul>
        </div>
        <div className="sidebarFooter">
            <div className="addCategoryWrapper">
                <input className="smallInput" placeholder="..." value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} />
      ...
      <main className="mainContent">
        <div className="contentWrapper">
            ...
            <div className="todoContainer">
                {todos.filter(t => !selectedCategory || t.categoryId === selectedCategory).map(todo => (
