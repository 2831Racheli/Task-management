...
function App() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
        ...
        Swal.fire({ icon: 'error', title: 'שגיאה', text: 'בדקי את פרטי ההתחברות' }); 
    }
  };

  // --- עדכונים אופטימיים (מהירות שיא) ---
  ...
  const handleAddTask = async (e) => {
    ...
    try {
        ...
        Swal.fire({ icon: 'error', text: 'ההוספה נכשלה' });
    }
  };
  ...
  const handleToggleComplete = async (todo, isComplete) => {
    const originalTodos = [...todos];
    // עדכון מצב ה-Checkbox על המסך מיד
  ...
  const handleAddCategory = async () => {
    ...
    try {
        ...
        Swal.fire({ icon: 'error', text: 'הוספת הנושא נכשלה' });
    }
  };
    ...
    return (
      <div style={styles.authContainer}>
        <div style={styles.authCard}>
          <h2 style={{ color: '#5D4F43', marginBottom: '25px' }}>
            {authData.isLogin ? "ברוכים הבאים" : "יצירת חשבון"}
          </h2>
